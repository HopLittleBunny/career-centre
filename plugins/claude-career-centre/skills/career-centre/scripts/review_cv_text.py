...
GENERIC_PHRASES = (
    "results-driven", "results oriented", "dynamic professional", "proven track record",
    "excellent communication skills", "hard-working", "hardworking", "team player",
    "strategic thinker", "detail-oriented", "fast-paced environment", "responsible for",
    "various stakeholders", "cutting-edge", "best-in-class", "leveraged synergies",
)
HEADING_GROUPS = {
...
def _finding(category: str, impact: str, message: str, recommendation: str, evidence: str | None = None) -> dict[str, Any]:
    item: dict[str, Any] = {"category": category, "impact": impact, "message": message, "recommendation": recommendation}
    if evidence:
...
def review_text(text: str, *, source: str = "supplied text", language: str = "en") -> dict[str, Any]:
    ...
    heading_lookup = {alias: group for group, aliases in HEADING_GROUPS.items() for alias in aliases}
    headings_found = sorted({heading_lookup[key] for line in lines if (key := _heading_key(line)) in heading_lookup}) if english_rules else []
    bullets = [BULLET_RE.sub("", line).strip() for line in lines if BULLET_RE.match(line)]
    duplicate_counts = Counter(re.sub(r"[^\w\s]", "", bullet.casefold()).strip() for bullet in bullets if len(bullet.split()) >= 5)
    ...
    sentences = [sentence.strip() for sentence in re.split(r"(?<=[.!?])\s+|\n+", text) if len(sentence.strip().split()) >= 8]
    long_sentences = [sentence for sentence in sentences if len(sentence.split()) > 38 and sentence.count("|") < 3]
    ...
    starter_counts = Counter(re.match(r"[A-Za-zÀ-ÖØ-öø-ÿ]+", bullet).group(0).casefold() for bullet in bullets if re.match(r"[A-Za-zÀ-ÖØ-öø-ÿ]+", bullet))
    repeated_starters = sorted(word for word, count in starter_counts.items() if count >= 3)
    findings: list[dict[str, Any]] = []
    strengths: list[str] = []
    if word_count < 80:
        findings.append(_finding("extraction", "high", "There is too little selectable text for a reliable CV review.", "Check that the file is readable and not an image-only scan, then provide the full CV text."))
    elif word_count < 180:
        findings.append(_finding("content depth", "medium", "The extracted CV is unusually brief, so important evidence may be missing.", "Check whether roles, dates, achievements or qualifications were lost during extraction."))
    else:
    ...
    if emails:
        ...
        findings.append(_finding("contact details", "high", "No email address was detected in selectable text.", "Add a current email address as real text rather than placing it only in an image or inaccessible header."))
    if not phones and word_count >= 180:
        findings.append(_finding("contact details", "low", "No phone number was detected in selectable text.", "Include one if it is appropriate for the target market, or keep it intentionally hidden as a confirmed preference."))
    if english_rules:
        if len(headings_found) >= 3:
            strengths.append("Common CV sections are clearly detectable in the extracted text.")
        elif word_count >= 180:
            findings.append(_finding("structure", "high" if len(headings_found) < 2 else "medium", "Few standard section headings were detected, which may make the CV harder to scan or parse.", "Use short, conventional headings for summary, experience, skills and education where those sections apply.", ", ".join(headings_found) if headings_found else "No common English section headings detected"))
    elif word_count >= 80:
    ...
    if date_signals >= 2:
        ...
        findings.append(_finding("chronology", "medium", "The review found few visible date signals.", "Make employment and education dates easy to associate with the correct role or qualification."))
    if len(bullets) >= 4:
    ...
    if long_sentences:
        findings.append(_finding("readability", "medium", f"{len(long_sentences)} sentence(s) exceed 38 words.", "Split the longest sentences so the action, context and result can be understood in one scan.", long_sentences[0]))
    if generic_hits:
        findings.append(_finding("specificity", "medium" if len(generic_hits) >= 2 else "low", "Generic career phrasing may be taking space from distinctive evidence.", "Replace only the flagged phrases with specific scope, decisions, stakeholders, outputs or outcomes that the source supports.", ", ".join(generic_hits[:6])))
    if len(passive_hits) >= 3:
        findings.append(_finding("ownership clarity", "low", "Several passive constructions may make ownership less clear.", "Review them manually and use active wording only where the evidence supports personal ownership.", f"{len(passive_hits)} possible passive constructions"))
    if raw_urls:
        findings.append(_finding("links", "low", "Visible raw web addresses were detected.", "In the final Word CV, use descriptive linked text where practical and confirm every link target.", raw_urls[0]))
    impact_order = {"high": 0, "medium": 1, "low": 2}
    findings.sort(key=lambda item: (impact_order[item["impact"]], item["category"]))
    status = "insufficient_text" if word_count < 80 else ("needs_attention" if any(item["impact"] in {"high", "medium"} for item in findings) else "clear")
    return {
        "source": source,
        ...
        "findings": findings,
        "metrics": {"word_count": word_count, "nonempty_lines": len(lines), "bullet_count": len(bullets), "headings_found": headings_found, "date_signals": date_signals, "outcome_or_scale_signals": outcome_signals, "email_addresses_detected": len(emails), "phone_numbers_detected": len(phones), "visible_raw_urls": len(raw_urls)},
        "limitations": ["This is a transparent writing and parseability diagnostic, not an employer ATS score.", "Every finding needs career-context and evidence review before editing."],
    }
