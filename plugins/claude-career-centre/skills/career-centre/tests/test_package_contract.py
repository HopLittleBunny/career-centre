...
class PackageContractTests(unittest.TestCase):
    ...
    def test_claude_manifest_matches_public_identity(self) -> None:
        manifest = json.loads((self.plugin_root / ".claude-plugin" / "plugin.json").read_text(encoding="utf-8"))
        self.assertEqual(manifest["name"], "career-centre")
        self.assertEqual(manifest["displayName"], "Career Centre")
        self.assertEqual(manifest["author"]["name"], "Amit Sharma")
        self.assertEqual(manifest["license"], "Apache-2.0")
        self.assertEqual(manifest["repository"], "https://github.com/HopLittleBunny/career-centre")
